    ...
    if ($("#checklogin").val() == 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $(this).css({ 'background': '#1c4fdb', 'pointer-events': 'inherit', 'cursor': 'pointer' });
            }
        });
    } else {
...
$(document).on('submit', '#report-form', function (e) {
   e.preventDefault();
   if ($(this).find('.report_message').val() != '') {
       $('.report_message').css('border', '1px solid #ccc');
       $('.report_message_error, .recaptcha_error, .report_update_message').html('');
       $.ajax({
           type: "POST",
           ...
           success: function (response) {
               if (response.success) {
                   $('.report_message').val('');
                   $('.report_update_message').html(response.msg).css('color', '#13A461');
                   //$('.report-sec').delay(6000).slideToggle();
               } else {
                   if (response.errors != '') {
                       $('.report_message_error').html(response.errors['report_message']);
                       $('.recaptcha_error').html(response.errors['g-recaptcha-response']);
                   }
                   $('.report_update_message').html(response.msg).css('color', '#ce2121');
               }
               $('input[name=csrf]').val(getCookie('csrf_cookie_name'));
           }
       });
   } else {
       $('.report_message').css('border', '1px solid #ce2121');
       $('.report_message_error').html('Please enter issue Message.');
   }
});
