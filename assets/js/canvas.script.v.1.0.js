...
function fontSize(item) {
    canvas.getActiveObject().setFontSize(item);
    canvas.renderAll();
}
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                vCanvas.loadFromDatalessJSON(response.template);
...
$(document).on('click', '.savemytemplate', function (e) {
    if ($("#checklogin").val() == 1) {
        ...
        $('.loader-msg').html('Saving template, please wait...');
        //var template_data = JSON.stringify(canvas.toJSON(['left', 'top', 'lockMovementX', 'lockMovementY']));
        canvas.backgroundImage = null;
        var template_file = canvas.toDataURL("image/png");
        JSONString_Objects = JSON.stringify(canvas.toJSON());
        canvas.setBackgroundImage(backgroundIMG, canvas.renderAll.bind(canvas), {
            scaleX: canvas.width / backgroundIMG.width,
            scaleY: canvas.height / backgroundIMG.height,
            originX: 'left',
        ...
        $.ajax({
            ...
            data: {
                ...
                'template_data': JSONString_Objects
            },
            dataType: "json",
            success: function (response) {
                if (response.success) {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                loadMyTemplates(response.userTemplates);
...
$(document).on('click', '.delete_custom_template', function (e) {
    e.preventDefault();
    var $this = $(this);
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        $this.parent('.template-item').remove();
                        Swal.fire({
                            title: "Deleted!",
...
function loadMyTemplates(templates) {
    var t = '',
        i = 0;
    console.log();
    for (i = 0; i < templates.length; i++) {
        t += '<div class="template-item"><a href="javascript:void(0);" class="user_template" data-template="' + templates[i].id + '" title="Custom Template"><img src="uploads/user_templates/' + templates[i].template_preview_file + '" alt="User Template"/></a><span class="delete_custom_template" data-template="' + templates[i].id + '" title="Delete this template"><i class="fa fa-trash"></i></span></div>';
    }
    $('.user_templates').append(t);
}
